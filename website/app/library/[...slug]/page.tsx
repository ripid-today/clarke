...
import { getFolders, getArticles, getArticleById } from "@/lib/firebase/firestore";
...
export default async function LibrarySlugPage({ params }: PageProps) {
  ...
  try {
    ...
    const folder = folders.find((f) => f.slug === slug[slug.length - 1]);
    ...
    // Try to find article by full path
    const article = await getArticleById(slugPath);
    ...
    if (article) {
      return (
        <div className="py-8 px-6">
          <Breadcrumbs
            items={[
              { name: "Folder", href: "/library" },
              { name: article.title, href: `/library/${slugPath}` },
            ]}
          />
